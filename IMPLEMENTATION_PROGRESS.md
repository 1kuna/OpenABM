...
- Operations lane: API scopes/RBAC, local API keys, local sessions, invite
  ...
  heartbeats, MCP observability, classified payload/business-dimension/code
  context exports, Docker Compose reference deployment, and admin status
...
| Make coherent commits as progress lands | `git log` shows focused slices on `main`, including classification access, export redaction, trend analytics, affected-entity review tasks, audit metadata, and completion-audit cleanup. | Complete |
...
- Added agent runtime configuration registry storage/API lifecycle: config
  ...
  pointers with tag events, deterministic config diffs, structured JSON-path
  changes, metadata diffs, tool-list deltas, config-section change buckets,
  linked eval summaries, and tag-movement history surfaced through API, OpenAPI,
  tests, and UI.
...
- Eval behavior-shift regression passed: eval run comparison now computes
  baseline/candidate behavior-label distributions from offline eval traces and
  reports match-count, status-count, trace-id, severity, and name deltas.
...
- Prompt registry and agent runtime configuration registry now have storage/API,
  ...
  agent config diffs now return tag-movement history; agent config diff also
  returns structured content and metadata diffs with tool/config-section deltas
  plus linked eval summaries; prompt diff also returns message-level diff when
  templates are JSON chat-message arrays and linked eval summaries for the
  compared commits.
...
- Added first-class trace-dimension and code-context classifications with
